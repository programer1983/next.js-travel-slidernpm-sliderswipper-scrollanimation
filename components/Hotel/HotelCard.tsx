...
const HotelCard = ({hotel}: Props) => {
  return (
    <div>
        ...
        <div>
            <h1 className='mt-4 text-lg font-semibold text-blue-950 hover:text-black transition-all duration-200 cursor-pointer'>
                {hotel.name}
            </h1>
            ...
            <div className='flex items-center space-x-2'>
                   ...
                   <p className='text-sm text-gray-800'>Expectional</p>
                   <p className='text-sm font-bold text-gray-800'>{hotel.reviews} Reviews</p>
            </div>
            <p className='mt-3 text-gray-700 font-medium'>
                Starting from <span className='text-blue-600 font-bold'>USD{hotel.price}</span>
            </p>
        </div>
    </div>
  )
